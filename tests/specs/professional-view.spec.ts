...
test.describe("Portfolio - Professional View", () => {
  ...
  test(
    "should display all CTA buttons in Professional view",
    { tag: "@regression" },
  ...
  test(
    "should open LinkedIn profile when LinkedIn button is clicked",
    { tag: "@regression" },
    async ({ browserContext }, testInfo) => {
      testInfo.annotations.push({ type: "TestCaseID", description: "TC_005" });

      // Arrange
      const linkedInButton = browserContext.page.locator(
  ...
  test(
    "should have correct mailto link for Email button",
    { tag: "@regression" },
    async ({ browserContext }, testInfo) => {
      testInfo.annotations.push({ type: "TestCaseID", description: "TC_006" });

      // Arrange
      const emailButton = browserContext.page.locator(
      ...
      // Assert
      expect(href).toBe("mailto:kousal.richard@gmail.com");
    },
  );

  test(
    "should initiate CV download when Download CV button is clicked",
    { tag: "@regression" },
    async ({ browserContext }, testInfo) => {
      testInfo.annotations.push({ type: "TestCaseID", description: "TC_007" });

      // Arrange
      const downloadButton = browserContext.page.locator(
